-- create_public_booking_public_v3 calculaba v_service_price/v_service_duration
-- leyendo únicamente el precio y la duración base de public_booking_services,
-- ignorando el override de precio/duración por profesional configurado en
-- Configuración → Equipo (business_settings.schedule._employeeServiceOverrides).
-- Resultado real: Alan con "Corte en cabello corto" a $25.000/60min -> la
-- Página Pública mostraba $25.000 (usa resolveServicePricing en el cliente,
-- src/lib/service-pricing.ts) pero el turno se insertaba con $21.900/40min
-- (el estándar del servicio), y Agenda/Caja leen ese valor ya congelado en
-- appointments.service_price/duration_min, no lo recalculan.
--
-- Se agrega acá la misma resolución de override que ya usa el cliente
-- (mismo criterio: useStandardPrice/useStandardDuration === false + valor
-- propio > 0 → usa el valor del profesional; si no, el estándar del
-- servicio). v_employee_overrides ya se leía en esta función (se usaba solo
-- para el switch "enabled"); ahora también se usa para precio/duración.
-- Nada más de la función cambia.
