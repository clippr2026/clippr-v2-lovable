-- Valida server-side el switch "Acepta reservas en línea" (perfil del
-- profesional en Configuración → Equipo), con el mismo criterio ya aplicado
-- en 20260826120000 para "Ofrece este servicio": hasta ahora solo se
-- respetaba filtrando en el cliente (src/routes/reservar/$slug.tsx), así que
-- se podía forzar una reserva con un profesional que tiene ese switch
-- apagado llamando al RPC directamente, sin pasar por la UI.
--
-- La visibilidad vive en business_settings.schedule._publicVisibility.employees
-- (JSONB { [employeeId]: boolean }), con fallback al campo legado
-- _employeeOnline para configuraciones guardadas antes de que existiera
-- _publicVisibility — exactamente el mismo criterio de resolución que usa
-- extractPublicVisibility() en el cliente (visibility.employees ?? source._employeeOnline).
-- Ausencia de entrada para el profesional = visible por default.
--
