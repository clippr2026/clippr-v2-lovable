-- Consolida "Acepta reservas en línea" en un solo switch: employees.is_active,
-- que es el campo que la vista public_booking_employees ya usa para filtrar
-- (ver v_has_employee más abajo). Se saca el chequeo separado contra
-- business_settings.schedule._publicVisibility.employees (agregado en
-- 20260826130000): era un segundo campo independiente que podía quedar
-- desincronizado de is_active y esconder (o exponer) profesionales de forma
-- inconsistente — exactamente el mismo problema de fondo que causó que un
-- profesional activo no apareciera en la página pública (caso Alan,
-- auro-stylo). El cliente (Configuración → Equipo) ahora tiene un único
-- switch "Acepta reservas en línea" que escribe employees.is_active.
--
-- No se modifica ningún otro comportamiento: el switch "Ofrece este
-- servicio" por profesional+servicio (20260826120000) sigue igual, así como
-- cálculo de precio/duración, detección de conflictos, alta/actualización
-- de cliente y creación de la reserva.
...
begin
  ...
  -- Único switch "Acepta reservas en línea" = employees.is_active. La vista
  -- public_booking_employees ya excluye is_active = false, así que este
  -- exists() es, en un solo paso, tanto "el profesional existe" como "acepta
  -- reservas en línea".
  select exists(
    select 1
