-- Segunda vuelta del fix de descansos en create_public_booking_public_v3.
-- La migración anterior (20260922010000) ya evitaba sumar el descanso del
-- negocio cuando el profesional tenía un HORARIO ESPECIAL para la fecha
-- exacta, pero seguía sumando el descanso del negocio al descanso SEMANAL
-- recurrente del profesional (el caso normal, sin fecha especial): si el
-- negocio tenía un breakStart/breakEnd propio Y el profesional también,
-- ambos se validaban — el mismo bug de "dos descansos" que ya se corrigió
-- del lado del cliente (`src/lib/availability.ts` `resolveDaySchedule`, que
-- ahora usa `applyBreakOverride` siempre, sin distinguir fecha especial vs
-- semanal). Acá se aplica el mismo criterio: si el profesional tiene
-- CUALQUIER horario propio para ese día (especial o semanal), su descanso
-- —o la ausencia de descanso— es el único que se valida; el descanso del
-- negocio solo se usa cuando el profesional no tiene ningún horario propio
-- configurado ese día (hereda todo del negocio).
