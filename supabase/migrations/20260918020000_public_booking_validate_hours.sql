-- create_public_booking_public_v3 validaba negocio/empleado activo,
-- visibilidad online, servicio habilitado y conflicto con turnos existentes
-- — pero NUNCA validaba que el horario elegido cayera dentro del horario
-- comercial del negocio ni del horario laboral del profesional. Un bug del
-- cliente (o una llamada directa al RPC) podía confirmar una reserva fuera
-- de horario sin que el servidor lo impidiera. Esta migración agrega esa
-- validación server-side, espejando la intersección negocio ∩ profesional
-- que ahora usa el motor de disponibilidad del cliente
-- (`src/lib/availability.ts` `resolveDaySchedule`), en la zona horaria real
-- del negocio (`businesses.timezone`). Si el horario configurado es
-- inválido o falta, la reserva se rechaza — nunca se asume un horario por
-- default.

create or replace function public._booking_parse_time_minutes(p_value text)
returns int
language sql
immutable
as $$
  select case
    when p_value ~ '^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$'
...
begin
  ...
  v_emp_day := coalesce(
    v_biz_schedule #> array['_employeeSpecialDates', p_employee_id::text, v_date_key],
    v_biz_schedule #> array['_employeeSchedules', p_employee_id::text, v_day_key]
  );
