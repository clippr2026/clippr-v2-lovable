-- create_public_booking_public_v3 validaba el descanso del negocio Y el
-- descanso propio del profesional como dos capas que se SUMAN (ambas
-- bloquean el slot). Pero un "Horario especial" cargado para
-- (profesional, fecha) es un reemplazo completo de ese día, no una capa
-- adicional: si el profesional apaga su descanso ese día puntual (o lo
-- edita), el descanso del negocio para esa fecha no debería seguir
-- bloqueando sus turnos. Mismo bug ya corregido en el motor de cliente
-- (`src/lib/availability.ts` `resolveDaySchedule`/`applyBreakOverride`):
-- ahora, si existe un horario especial del profesional para la fecha exacta,
-- SOLO se valida el descanso de ese override — no el del negocio ni el del
...
begin
  ...
  if v_emp_special_day is not null then
    -- Horario especial del profesional para ESTA fecha exacta: override
    -- completo — su descanso (o la ausencia de descanso) manda, sin sumar
    -- el descanso del negocio ni el del horario semanal recurrente.
    v_break_start := public._booking_parse_time_minutes(v_emp_special_day ->> 'breakStart');
    v_break_end := public._booking_parse_time_minutes(v_emp_special_day ->> 'breakEnd');
    ...
    if v_emp_weekly_day is not null then
      v_break_start := public._booking_parse_time_minutes(v_emp_weekly_day ->> 'breakStart');
      v_break_end := public._booking_parse_time_minutes(v_emp_weekly_day ->> 'breakEnd');
      if v_break_start is not null and v_break_end is not null and v_break_end > v_break_start
         and v_slot_start_min < v_break_end and v_slot_end_min > v_break_start
      then
        raise exception 'Ese horario cae dentro del descanso configurado.' using errcode = 'P0001';
