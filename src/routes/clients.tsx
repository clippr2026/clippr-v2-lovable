    ...
    <AppShell>
      ...
      <div className="space-y-6 animate-fade-up">
        <div className="grid gap-3 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          <StatCard featured label="VIP" value={String(counts.vip)} caption="+ gasto + visita" link="Ver todos" onLinkClick={() => showGroup("VIP", (c) => c.status === "vip")} icon={<Crown className="h-7 w-7 text-violet-300" />} glow="bg-violet-500/25" />
          <StatCard label="Nuevos" value={String(counts.nuevos)} caption="ingresaron este mes" link="Ver todos" onLinkClick={() => showGroup("Nuevos", (c) => c.status === "nuevo")} icon={<Sparkles className="h-7 w-7 text-violet-300" />} glow="bg-violet-400/20" />
          ...
          <StatCard label="Activos" value={String(counts.activos)} caption={<span className="inline-flex items-center gap-2">visita en <input type="number" min={1} value={daysActive} onChange={(e) => setDaysActive(sanitizeDays(e.target.value))} className="w-12 rounded-md bg-white/5 ring-1 ring-white/10 px-1.5 py-0.5 text-foreground text-center" /> días</span>} link="Ver todos" onLinkClick={() => showGroup("Activos", (c) => c.status === "activo")} icon={<CheckCircle2 className="h-7 w-7 text-emerald-300" />} glow="bg-emerald-400/20" />
          <StatCard label="Inactivos" value={String(counts.inactivos)} caption={<span className="inline-flex items-center gap-2">sin visita + <input type="number" min={1} value={daysInactive} onChange={(e) => setDaysInactive(sanitizeDays(e.target.value))} className="w-12 rounded-md bg-white/5 ring-1 ring-white/10 px-1.5 py-0.5 text-foreground text-center" /> días</span>} link="Ver todos" onLinkClick={() => showGroup("Inactivos", (c) => c.status === "inactivo")} icon={<PauseCircle className="h-7 w-7 text-muted-foreground" />} glow="bg-white/10" />
          <StatCard label="Perdidos" value={String(counts.perdidos)} caption={<span className="inline-flex items-center gap-2">sin visita + <input type="number" min={1} value={daysLost} onChange={(e) => setDaysLost(sanitizeDays(e.target.value))} className="w-12 rounded-md bg-white/5 ring-1 ring-white/10 px-1.5 py-0.5 text-foreground text-center" /> días</span>} link="Reconquistar" onLinkClick={() => showGroup("Perdidos", (c) => c.status === "perdido")} icon={<AlertTriangle className="h-7 w-7 text-rose-300" />} glow="bg-rose-400/20" />
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[400px_1fr]">
          <div className="glass rounded-2xl p-4 flex flex-col gap-4 max-h-[82vh]">
            ...
            <div className="flex-1 overflow-y-auto pr-1 -mr-1 space-y-2">
              {filtered.map((c, i) => <button key={c.id} onClick={() => setSelected(c.id)} className={cn("w-full flex items-center gap-3 rounded-xl p-3 ring-1 transition text-left relative overflow-hidden", selected === c.id ? "bg-gradient-to-r from-sky-500/15 via-violet-500/8 to-transparent ring-violet-400/40" : "bg-white/[0.02] ring-white/5 hover:bg-white/5")}><div className={cn("h-11 w-11 rounded-full grid place-items-center text-sm font-semibold bg-gradient-to-br ring-1", avatarTints[i % avatarTints.length])}>{c.name[0]?.toUpperCase()}</div><div className="flex-1 min-w-0"><div className="flex items-center justify-between gap-2"><div className="font-medium truncate">{c.name}</div><div className="text-sm font-semibold tabular-nums">${c.spent.toLocaleString("es-AR")}</div></div><div className="text-[11px] text-muted-foreground truncate">{c.visits} visitas · {c.lastVisit ?? "sin visitas"}</div><div className="flex items-center justify-between mt-1.5"><Rating value={c.rating} />{statusBadge(c.status)}</div></div></button>)}
          ...
          <div className={cn("glass rounded-2xl p-0 min-h-[60vh] overflow-hidden", current?.status === "perdido" && "ring-1 ring-rose-400/30", current?.status === "vip" && "ring-1 ring-amber-300/30")}>
            {current ? <div className="flex flex-col h-full">
              ...
              <div className="p-6 flex-1 space-y-5">
                {tab === "resumen" && <><div className="grid grid-cols-2 md:grid-cols-4 gap-3"><div className="rounded-xl bg-gradient-to-br from-sky-400/15 to-violet-500/8 ring-1 ring-violet-400/30 p-3"><div className="text-[10px] uppercase tracking-[0.18em] text-violet-200/90">Lifetime value</div><div className="text-2xl font-display mt-1 tabular-nums">${current.spent.toLocaleString("es-AR")}</div><div className="text-[10px] text-muted-foreground mt-0.5 inline-flex items-center gap-1"><TrendingUp className="h-3 w-3 text-emerald-300" /> gasto total</div></div><div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-3"><div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Visitas</div><div className="text-2xl font-display mt-1 tabular-nums">{current.visits}</div><div className="text-[10px] text-muted-foreground mt-0.5">total histórico</div></div><div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-3"><div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Ticket prom.</div><div className="text-2xl font-display mt-1 tabular-nums">${ticket.toLocaleString("es-AR")}</div><div className="text-[10px] text-muted-foreground mt-0.5">por visita</div></div><div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-3"><div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Última visita</div><div className="text-sm font-semibold mt-2">{current.lastVisit ?? "—"}</div><div className="text-[10px] text-muted-foreground mt-0.5 inline-flex items-center gap-1"><Clock3 className="h-3 w-3" /> {current.lastVisit ? `Últ: ${current.lastVisit}` : "sin visitas"}</div></div></div><div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-4"><div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground mb-2">Notas</div><textarea value={noteDraft} onChange={(e) => setNoteDraft(e.target.value)} placeholder="Agregá notas internas del cliente..." className="w-full min-h-[110px] rounded-xl bg-white/5 ring-1 ring-white/10 p-3 text-sm focus:outline-none focus:ring-violet-400/40" /><div className="flex justify-end mt-3"><button onClick={saveNotes} disabled={updateNotes.isPending} className="rounded-xl bg-gradient-to-r from-sky-400 to-violet-500 text-background px-4 py-2 text-sm font-semibold disabled:opacity-50">{updateNotes.isPending ? "Guardando…" : "Guardar nota"}</button></div></div></>}
