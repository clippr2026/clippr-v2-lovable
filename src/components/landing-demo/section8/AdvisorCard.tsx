...
export const AdvisorCard = React.forwardRef<HTMLDivElement>(function AdvisorCard(_props, ref) {
  return (
    <div ref={ref} className="flex w-full flex-col gap-3">
      <div className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-5 shadow-[0_30px_90px_-40px_rgba(0,0,0,0.9)] backdrop-blur-xl sm:p-6 lg:p-7">
        ...
        <div className="mt-1 flex items-baseline gap-1 font-display text-5xl font-bold lg:text-6xl">
      ...
      <div className="mt-2 text-sm font-medium text-white/80">Recomendaciones para vos</div>
      <div className="flex flex-col gap-3">
        {RECOMMENDATIONS.map((r) => (
          <div
            key={r.title}
            className="s8-reco rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-4 backdrop-blur-xl sm:p-5"
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="flex flex-1 items-start gap-3 sm:items-center">
                <span
                  className="s8-reco-icon mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full sm:mt-0"
                  style={{
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {r.lines.map((line) => (
                    <div key={line} className="mt-0.5 text-xs text-white/50">
                      {line}
                    </div>
                  ))}
                </div>
              ...
              <span
                className="s8-cta inline-flex shrink-0 items-center justify-center gap-1.5 self-end rounded-full px-3.5 py-1.5 text-xs font-semibold text-white sm:self-center"
                style={{ background: "linear-gradient(135deg, oklch(0.62 0.24 292), oklch(0.46 0.24 296))" }}
              >
                {r.cta}
                <ArrowRight className="s8-cta-arrow h-3.5 w-3.5" />
              </span>
            </div>
          </div>
        ))}
